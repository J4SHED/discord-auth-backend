...
app.post('/api/auth/discord', async (req, res) => {
    ...
    try {
        ...
        db.get('SELECT * FROM users WHERE discord_id = ?', [user.id], async (err, row) => {
            ...
            if (!row) {
                ...
                db.run(
                    `INSERT INTO users (discord_id, username, trial_start, trial_expiry, renewal_count, last_login, status) VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [user.id, user.username, now.toISOString(), trialExpiry.toISOString(), 0, now.toISOString(), 'active']
                );
            } else {
                trialExpiry = new Date(row.trial_expiry);
                renewalCount = row.renewal_count;
                status = row.status || 'active';
                db.run(`UPDATE users SET last_login = ?, username = ? WHERE discord_id = ?`, [now.toISOString(), user.username, user.id]);
            }
